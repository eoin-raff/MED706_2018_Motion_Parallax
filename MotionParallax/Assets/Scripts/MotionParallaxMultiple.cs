...
public class MotionParallaxMultiple : MonoBehaviour {
    ...
    public int display;                         //Which display this is attached to
    ...
    void LateUpdate()
	{
        if (eyes != null)
        {
            ...
            GetWindowPosition(mainCamera, trackedEyePosition, display);
        }
    }
    ...
    void GetWindowPosition(Camera cam, Vector3 perspectiveOffset, int display)
	{
        float shiftL = -0.5f;
        float shiftR = 0.5f;
        ...
        float left = cam.nearClipPlane * (shiftL * aspectRatio - perspectiveOffset.x) / Mathf.Abs(perspectiveOffset.z);
        float right = cam.nearClipPlane * (shiftR * aspectRatio - perspectiveOffset.x) / Mathf.Abs(perspectiveOffset.z);
